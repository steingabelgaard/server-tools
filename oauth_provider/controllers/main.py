...
class OAuth2ProviderController(http.Controller):
    ...
    @http.route(
        "/oauth2/token", type="http", auth="none", methods=["POST"], csrf=False
    )
    def token(
        self,
        client_id=None,
    ...
    def tokeninfo(self, access_token=None, *args, **kwargs):
        ...
        if (
            not access_token
            and "HTTP_AUTHORIZATION" in http.request.httprequest.environ
        ):
            access_token = http.request.httprequest.environ[
                "HTTP_AUTHORIZATION"
            ].split()[1]
